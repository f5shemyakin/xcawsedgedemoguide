...
With the **New VPC Parameters** selected by default, go on to create a new VPC. Select **Autogenerate VPC Name** and enter the **10.125.0.0/16** CIDR in the IPv4 CIDR block field.
In this step, we will create an app stack cluster. Open the **Select Ingress Gateway or Ingress/Egress Gateway** menu, and select **App Stack Cluster (One Interface)**. It will use single interface and be used for deploying K8s cluster. 
